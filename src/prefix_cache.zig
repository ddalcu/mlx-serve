...
pub const HotPrefixCache = struct {
    ...
    pub fn lookupAndRestoreWithMedia(
        ...
        return self.lookupAndRestoreMediaHistory(target_cache, target_moe_seq_offset, target_ssm_entries, s, prompt_ids, has_tools, .{ .key = vision_key, .start = media_start }, dflash_target, mtp_target, slot_id, skip);
    }
    ...
    pub fn commitWithMediaState(
        ...
        return self.commitMediaHistory(source_cache, tokens, has_tools, cache_key, .{ .key = vision_key, .start = media_start }, ssm_cps, dflash, mtp, prompt_len);
    }
...
test "HotPrefixCache: appended media retains post-image hybrid checkpoints" {
    ...
    _ = try hc.commitMediaHistory(&source, tokens[0..10], false, 0, .{ .key = 1, .start = 2, .spans = &.{a} }, cps, null, null, 10);
    ...
    const hit = try hc.lookupAndRestoreMediaHistory(&target, &offset, &restored, s, &tokens, false, next, null, null, null, false);
    ...
    const miss = try hc.lookupAndRestoreMediaHistory(&target, &offset, &restored, s, &tokens, false, .{ .key = 3, .start = 2, .spans = &.{ edited, b } }, null, null, null, false);
