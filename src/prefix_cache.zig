...
pub const HotPrefixCache = struct {
    ...
    fn findBestMatchWithMedia(self: *const HotPrefixCache, prompt_ids: []const u32, has_tools: bool, vision_key: u64, media_start: ?usize, quant_config: kv_quant.KVQuantConfig) ?struct { idx: usize, shared: usize } {
    ...
    fn findBestMatch(self: *const HotPrefixCache, prompt_ids: []const u32, has_tools: bool, vision_key: u64, quant_config: kv_quant.KVQuantConfig) ?struct { idx: usize, shared: usize } {
        const match = self.findBestMatchWithMedia(prompt_ids, has_tools, vision_key, null, quant_config) orelse return null;
    ...
    pub fn lookupAndRestoreWithMedia(
        ...
        const match = self.findBestMatchWithMedia(prompt_ids, has_tools, vision_key, media_start, target_cache.config);
