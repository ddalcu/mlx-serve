...
pub fn scriptFor(allocator: std.mem.Allocator, kind: AgentKind, base_url: []const u8, model: []const u8, budget: Budget, opencode_config: ?[]const u8, extras: []const []const u8) ![]u8 {
    ...
    switch (kind) {
        ...
        .codex => {
            ...
            // auth) is used as-is, and the generated `--profile mlx-serve`
            // layer carries our keys. PATH first, then the CLI the desktop app
            // bundles (codex's rebranded app installs as ChatGPT.app or
            // Codex.app, bundle id com.openai.codex, CLI at
...
fn writeCodexProfile(allocator: std.mem.Allocator, io: std.Io, base_url: []const u8, model: []const u8, budget: Budget) !void {
    ...
    defer dir.close(io);
    // Only a missing file means "fresh"; oversized or unreadable must fail
    // loudly, not silently drop the user's lines.
    const existing = dir.readFileAlloc(io, "mlx-serve.config.toml", allocator, .limited(1 << 20)) catch |err| switch (err) {
...
test "codex config: codex-owned [projects.*] tables survive the rewrite" {
    const existing =
        ...
        \\
    ;
    const toml = try codexConfigToml(t.allocator, "http://x:2", "m2", .{ .context = 8192, .output = 4096 }, existing);
    defer t.allocator.free(toml);
...
test "codex config: a file whose first line is a table header gets the root keys first" {
    const existing =
        \\[projects."/x"]
        \\trust_level = "trusted"
        \\
    ...
    const toml = try codexConfigToml(t.allocator, "http://x:2", "m2", .{ .context = 8192, .output = 4096 }, existing);
    defer t.allocator.free(toml);
    // root keys belong ABOVE the first table header, inserted there
    try t.expect(std.mem.indexOf(u8, toml, "model_context_window = 8192").? < std.mem.indexOf(u8, toml, "[projects.").?);
    try t.expect(std.mem.indexOf(u8, toml, "model = \"m2\"") != null);
    try t.expect(std.mem.indexOf(u8, toml, "model_provider = \"mlx\"") != null);
    try t.expect(std.mem.indexOf(u8, toml, "[projects.\"/x\"]\ntrust_level = \"trusted\"") != null);
}

test "codex config: an existing file without a trailing newline ends with exactly one" {
    const existing = "model = \"old\"\nmodel_provider = \"mlx\"\nmodel_context_window = 4096";
    const toml = try codexConfigToml(t.allocator, "http://x:2", "m2", .{ .context = 8192, .output = 4096 }, existing);
    defer t.allocator.free(toml);
    try t.expect(std.mem.indexOf(u8, toml, "model = \"old\"") == null);
    try t.expect(std.mem.endsWith(u8, toml, "\n"));
    try t.expect(!std.mem.endsWith(u8, toml, "\n\n"));
}

test "codex config: [[array]] headers decline the mlx body scan" {
    // A `[[model_providers.mlx]]` array header is declined, so the whole
    // table is appended after it — never merged into the array's scope.
    const existing =
        \\model = "old"
        \\
        \\[[model_providers.mlx]]
    ...
    try t.expect(std.mem.indexOf(u8, toml, "model = \"m2\"") != null);
    try t.expect(std.mem.indexOf(u8, toml, "[[model_providers.mlx]]\nname = \"foreign\"") != null);
    try t.expectEqual(@as(usize, 1), std.mem.count(u8, toml, "\n[model_providers.mlx]\n"));
}

test "codex config: a whitespace-only existing file gets the fresh profile" {
    const fresh = try codexConfigToml(t.allocator, "http://x:2", "m2", .{ .context = 8192, .output = 4096 }, "");
    defer t.allocator.free(fresh);
    const blank = try codexConfigToml(t.allocator, "http://x:2", "m2", .{ .context = 8192, .output = 4096 }, "  \n\t\n");
    defer t.allocator.free(blank);
    try t.expectEqualStrings(fresh, blank);
}
...
test "codex profile file lands in the effective home, base config untouched" {
    ...
    try writeCodexProfile(t.allocator, std.testing.io, "http://x:1", "m1", .{ .context = 4096, .output = 2048 });
    ...
    const expected = try codexConfigToml(t.allocator, "http://x:1", "m1", .{ .context = 4096, .output = 2048 }, "");
    defer t.allocator.free(expected);
    try t.expectEqualStrings(expected, written);
    // Overwrite at the next launch works the same way.
    try writeCodexProfile(t.allocator, std.testing.io, "http://x:2", "m2", .{ .context = 8192, .output = 4096 });
...
test "codex profile: an oversized existing profile fails, never a silent rewrite" {
    var dir = std.testing.tmpDir(.{});
    defer dir.cleanup();
    var root_buf: [std.fs.max_path_bytes]u8 = undefined;
    const root_len = try dir.dir.realPath(std.testing.io, &root_buf);
    ...
    try std.Io.Dir.cwd().createDirPath(std.testing.io, home_z);
    var d = try std.Io.Dir.openDirAbsolute(std.testing.io, home[0..std.mem.len(home)], .{});
    defer d.close(std.testing.io);
    const huge = try t.allocator.alloc(u8, (1 << 20) + 1);
    defer t.allocator.free(huge);
    @memset(huge, 'x');
    try d.writeFile(std.testing.io, .{ .sub_path = "mlx-serve.config.toml", .data = huge });

    try t.expectError(error.StreamTooLong, writeCodexProfile(t.allocator, std.testing.io, "http://x:1", "m1", .{ .context = 4096, .output = 2048 }));
    // The user's profile survives the failed launch untouched.
    const after = try d.readFileAlloc(std.testing.io, "mlx-serve.config.toml", t.allocator, .limited(1 << 21));
    defer t.allocator.free(after);
    try t.expectEqual(huge.len, after.len);
}
