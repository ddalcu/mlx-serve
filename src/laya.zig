...
pub const Questions = struct {
    ...
    pub fn init(a: std.mem.Allocator, questions: std.json.Value) !Questions {
        if (questions != .object) return error.QuestionsNotObject;
        const qobj = questions.object;
        if (qobj.count() == 0) return error.NoQuestions;
        ...
        while (it.next()) |kv| : (n += 1) {
            ids[n] = kv.key_ptr.*;
            qs[n] = try Question.fromJson(a, kv.value_ptr.*);
        }
...
pub const Model = struct {
    ...
    /// Output columns `[lo, hi)` of `l`, copied contiguous.
    fn sliceOut(self: *Model, l: Linear, lo: c_int, hi: c_int) !Linear {
        const rows = mlx.mlx_array_shape(l.w_t)[0];
        var view = mlx.mlx_array_new();
        defer free(view);
        try mlx.check(mlx.mlx_slice(&view, l.w_t, &[_]c_int{ 0, lo }, 2, &[_]c_int{ rows, hi }, 2, &[_]c_int{ 1, 1 }, 2, self.stream));
        var w = mlx.mlx_array_new();
        errdefer free(w);
        try mlx.check(mlx.mlx_contiguous(&w, view, false, self.stream));
        try self.owned.append(self.allocator, w);
        const b = if (l.b) |bias| blk: {
            var out = mlx.mlx_array_new();
            errdefer free(out);
            try mlx.check(mlx.mlx_slice(&out, bias, &[_]c_int{lo}, 1, &[_]c_int{hi}, 1, &[_]c_int{1}, 1, self.stream));
...
/// `MAX_ROWS` rows and `MAX_TOKENS` padded tokens, where a longer row may add
/// at most `MAX_PAD_GROWTH` padding tokens to the rows already in the batch.
...
pub const Engine = struct {
    ...
    /// a non-finite row of its own); a failed forward fails the jobs that had
    /// rows in it.
    ...
    fn runJobs(self: *Engine, jobs: []const Job, works: []Work) !void {
        ...
        for (plan.ends) |b_end| {
            ...
            for (batch, 0..) |ri, r| {
                ...
                softmaxInto(logits, @max(1e-3, scale), w.probs[row.q]);
                w.acts[row.q] = act[0];
            }
        }
    }
...
test "laya: requests answered in one pass match their serial answers; errors stay per request" {
    ...
    for (items, &qs) |it, *q| q.* = try Questions.init(a, it.object.get("questions").?);
