...
pub const Engine = struct {
    ...
    pub fn predictJson(self: *Engine, a: std.mem.Allocator, model_id: []const u8, state: std.json.Value, questions: std.json.Value) ![]u8 {
        ...
        while (it.next()) |kv| : (qi += 1) {
            ...
            nqs += 1;
            seqs[qi] = try buildSequence(a, &self.model.tok, &self.model.tok_cache, cfg, state_ids, &qs[qi]);
            nseq += 1;
        ...
        for (plan.ends) |b_end| {
            ...
            for (rows, 0..) |i, r| {
                ...
                softmaxInto(res.logits[r * res.k_pad .. r * res.k_pad + k], @max(1e-3, scale), probs[i]);
