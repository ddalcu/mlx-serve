...
test "parseConfigFromJson: qwen3_vl remaps the family, captures the original type and parses the DeepStack indexes" {
    ...
    // (c) land vision_config keys in qv_* INCLUDING deepstack_visual_indexes
    // (the embedding model ships [5, 11, 17]), and (d) read M-RoPE sections
    // from text_config.rope_scaling. No allocator-owned fields — the
    ...
    try testing.expectEqual(@as(usize, 3), config.qv_deepstack_count);
    try testing.expectEqual(@as(u32, 5), config.qv_deepstack_indexes[0]);
    try testing.expectEqual(@as(u32, 11), config.qv_deepstack_indexes[1]);
    try testing.expectEqual(@as(u32, 17), config.qv_deepstack_indexes[2]);
