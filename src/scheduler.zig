...
fn commitSlotIfApplicable(sch: *Scheduler, slot: *Slot) void {
    ...
    const finish_st = hc.commitMediaHistory(&slot.cache, total_tokens, slot.has_tools, slot.cache_key, .{ .key = slot.vision_key, .start = slot.media_start, .spans = slot.media_spans }, ssm_cps_opt, dflash_commit, mtp_commit, slot.full_prompt.len) catch |err| {
...
fn commitCancelledPrefillSlot(slot: *Slot, hc: *prefix_cache_mod.HotPrefixCache) void {
    ...
    const st = hc.commitMediaHistory(&slot.cache, slot.full_prompt[0..len], slot.has_tools, slot.cache_key, .{ .key = slot.vision_key, .start = media_start, .spans = slot.media_spans }, cps, null, null, len) catch |err| {
