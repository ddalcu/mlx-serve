...
- `VStack`. Exact heights cost ~60 MB across ten long chats; wrong heights cost every scroll decision in the file.
...
- A shrinking row keeps the control the reader clicked where it is: the offset drops by what the row lost (`rowWillShrink` snapshots offset and height, `contentGeometry` corrects per frame, `rowDidShrink` ends it). The size-change anchor cannot do this: it holds the end only when you are already there.
