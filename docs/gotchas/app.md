...
**And a third door: the tray.** "Start Server" passed `selectedModelPath` unconditionally and was disabled without one, so a tray-started server always held a checkpoint — and ejecting that checkpoint brought it straight back, which reads as the app fighting you. It is the server doing what it was told: `--model` makes that entry the registry's default, so the next request reloads it, while a headless server has no default to restore. The button now asks the same setting the checkbox does (`StartupModelChoice.trayStartPath`), `ServerManager.toggle` reads an empty path as headless rather than launching a `--model` with no model, and the disable is gated on a start that actually loads — a Mac with only media models, or only LAN peers, has every reason to want a server and nothing to put in it. The button also stops saying "Loading Model..." for a start that loads nothing.

**A fourth: the chat window's own Start.** It called `ensureServerForLan()`, which passes `--model`, so a chat started from a dead composer inherited the same sticky default. `AppState.startServerForChat` starts headless and then hot-loads the selection through `ServerManager.ensureDefaultChatModel` — the same resident model, reached as an ordinary registry entry that stays ejected when you eject it. The load is kicked there rather than left to the first turn because the user just pressed a button and is owed a spinner: `loadingModelPath` is what makes the pill name the model and spin, under the same generation guard a hot-switch uses so a stale task cannot clear a newer one's flag. Nothing locally selected means the model answering is on another Mac, and a headless server is all the proxy needs. The pill needed no change — its dot already goes orange for a running server with nothing resident, which the headless world simply made common.
