...
Live report: a user's MLX Core session hung mid-chat after 3-4 turns of RAG-style tool calling. `mlx-serve` itself was idle and healthy (`/health` sub-2ms, `requests_running=0`) and `~/.mlx-serve/last-agent-request.json` hadn't been touched in 11+ minutes, ruling out both the server and the earlier-documented "ghost turn" pad-retry class (which keeps rewriting that file). `ps`/`top` on the app process (`MLXCore`) showed ~160% average CPU sustained over a 23h uptime with brief spikes to ~700%, and 5326 threads — not normal for an idle chat app. `sample <pid> 1 -f /tmp/sample.txt` showed two threads with 657/1000 and 655/1000 samples respectively sitting inside `closure #1 in Client.connect(transport:)`, awaiting an `AsyncThrowingStream` iterator via a stdio transport actor. A healthy, successfully-connected MCP session would have RETURNED from `client.connect(transport:)` already (the call completes once the initialize handshake finishes) — a thread still inside it after 23h means that specific connect attempt never completed.

Root cause: `MCPManager.connectOrFailFast` (then `private`, `app/Sources/MLXServe/Services/MCPManager.swift`) races `client.connect(transport:)` (Path A) against a death-watcher (Path B, polls `child.isRunning` at 10 Hz) and a hard 30s timeout. Whichever of B/timeout wins resumes the outer continuation and `spawnAndConnect` throws — but Path A's `Task` is never cancelled, only abandoned, per the function's own doc comment ("the loser keeps running but we abandon awaiting it"), a deliberate tradeoff to avoid `withThrowingTaskGroup`'s destructor hanging on an unresponsive child (see that comment for the full reasoning). The gap: nothing on the losing paths ever called `client.disconnect()`. Confirmed against the vendored SDK (`app/.build/checkouts/swift-sdk/Sources/MCP/Client/Client.swift:287-320`): `disconnect()` is what resumes every pending request continuation (`request.resume(throwing: MCPError.internalError("Client disconnected"))`) and is the ONLY thing that does — cancelling the Task or killing the child process does not reliably unstick a `withCheckedThrowingContinuation` already waiting on a JSON-RPC reply. `executeToolCall`'s watchdog, a few hundred lines above in the same file, already documents this exact SDK behavior ("that continuation does not honor task cancellation... The reliable break-glass is `client.disconnect()`") and already applies it correctly for tool CALLS — the connect-time race just never got the same treatment.

Each server that failed to answer `initialize` within 30s (or died slowly enough that Path B's poll still lost to a slow reply) left one more of these leaked, permanently-scheduled tasks — accumulating over a long-running app session and explaining both the CPU creep and the abnormal thread count. Since the leak lives entirely on the app side, `mlx-serve` stayed idle and healthy throughout; the chat only "hung" because the agent's next turn was waiting on a tool call routed through the same starved cooperative thread pool.

Fix: both losing paths (`Path B`'s death-watcher and the hard-cap timer) now call `client.disconnect()` (via `Task.detached` so it doesn't block the continuation-resume closure) before resuming, with the timeout path also calling `child.terminate()` first in case the process is still alive. `connectOrFailFast` gained an injectable `hardCapSeconds` (default unchanged at 30) and an `onPathASettled` test hook so the regression test can force the timeout path in well under a second instead of waiting out 30s, and dropped its `private` modifier to `internal` so `@testable import MLXCore` can drive it directly. Guard: `MCPConnectLeakTests.testAbandonedConnectSettlesAfterLosingTheRace` — spawns a real `sleep 60` child (accepts the stdio pipes, never speaks MCP so the death-watcher never fires), forces a 0.3s hard cap, and asserts the abandoned Path A task settles within 5s instead of never.

Rule: any race that abandons one arm on the other's win must ask whether the abandoned arm can ever unstick itself — for this SDK's checked continuations, only `disconnect()` does. A "loser keeps running" comment in a race is a code smell worth re-reading every time a related watchdog is added nearby, in case the treatments have quietly diverged (as they did here for ~months between the tool-call watchdog and the connect race). Diagnosis path worth repeating: `/health` + `requests_running`/`gpu_utilization_pct` from `/metrics.json` rule out the server in under a second; `sample <pid> 1 -f <path>` on the app process is the fastest way to catch a CPU-spinning leak red-handed, and a thread still inside a one-shot async call (like `connect`) hours into a session is itself the tell — a successfully completed call would have returned.
