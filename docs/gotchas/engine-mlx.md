...
The plan's premise was "verification rows bottlenecked by routed expert banks lacking NAX optimization." Half of it is true: the per-block profiler (deltas-only) puts the MoE at ~60% of the verify-row marginal on M5 Max (+4.31 ms/row of +7.12 at S1→S2, +2.41 of +4.13 at S2→S4), with hc/gdn/attn all small after the row-batched kernels. The other half is not: a grouped-expert m16 NAX kernel — one threadgroup per equal-expert run of the sorted pairs, streaming the gate and up banks through the tensor-ops tile with the table SwiGLU fused — passes fp32-truth parity on every group shape and `test_mtp_equivalence.sh` 11/11 armed, and LOSES ~9% on the forward: S=2 20.29 vs 18.66 ms, S=4 27.12 vs 24.91 (v1, two sequential K passes at NSG 8); 20.29/26.96 (v2, single pass, dual accumulators, NSG 4 — the barrier/pass halving moved nothing, so the cost is not synchronization).

The structural read: at verify widths a run is 1-2 rows, so the m16 tile's row amortization never engages while its costs — hundreds of small threadgroups each walking a full K loop for a 32-column window, JIT'd at MathMode::Safe against a tuned metallib — are fully paid, and the SORTED gather_qmm is already near the expert-byte floor (~1.7 ms/row: 10 experts x 2.46 MB x 48 layers / ~700 GB/s). NAX is not the missing piece for routed experts; the bytes are irreducible without cross-row expert overlap, which S<=9 with top-10-of-many routing does not produce. The declining mlp marginal at wider S (4.31 → 2.41) is that overlap slowly arriving — a hypothetical S=16+ regime could revisit this, which is why the lane ships OPT-IN default-OFF (`MLX_SERVE_MOE_VERIFY_NAX=1`, crossrow-lane precedent: measured, documented, off) instead of being deleted. Anyone tempted by a "fuse the whole verify MoE" variant should start from these numbers: the multi-row gather kernel (+38% at S=4) and the per-row gatherQmv loop (−11% at S=4) are the same lesson from other angles.
