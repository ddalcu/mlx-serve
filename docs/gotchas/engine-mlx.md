...
`MtpHeadRef.costProfile`'s `.qwen4` arm hard-coded `.generic`, so on an M5 Max the Flash-Next controller priced verify positions at the M4-fitted default (lo .10/hi .26) while the real marginals are three to five times steeper. Forced-depth saturated echo on the 4-bit pack (M5 Max 40-core, depths {1,2,3,4,6,8}, two reversed passes, 3 reps, medians of per-request `round_ms`): T(1)=23.61, T(2)=28.12, T(3)=33.91, T(4)=38.87, T(6)=50.16, T(8)=62.70 ms. Least squares over region counts: 18.24 ms floor, composite marginals .283/.310/.344, SSE 0.28, T(8)/T(6) reproduced exactly (1.25).

Two structural facts fall out. A verify row on this arch is BYTES — each extra row reads its own experts — so the marginal never flattens; and there is NO NAX takeover at M>=8: the routed expert banks never ride the vqmm NAX lane (dense projections do; the grouped-expert kernel is the M5 plan's item 2, and this table is its before-number). `nax_from=7` in the new `MTP_EV_G17_NAX_QWEN4_Q4_GS64_COSTS` therefore encodes a measured k>=7 STEEPENING (+11%/pos), not a discount — the third region's field name is about position, not blessing.
