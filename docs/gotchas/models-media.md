...
**LTX first/last frame.** I2V pinned one image as latent frame 0 via `cond_mask` + clean latent. The last anchor is the same `VideoConditionByLatentIndex` at `latent_idx = F_lat-1` — the last latent frame IS the last pixel frame on the causal VAE (`num_frames = 8k+1`). `keyframeMask(HW, F_lat, has_first, has_last)` is the one place the ranges live, `buildKeyframeCond` splices each present anchor over the noise and the clean latent (both pipelines; two-stage at the half and full grids). The 2.5 `keyframes_abs_pos_embedding` already derived from the mask, so the last anchor picks it up with no new wiring. Note what upstream does NOW: `combined_image_conditionings` replaces only `frame_idx == 0` and routes every other index through `VideoConditionByKeyframeIndex`, which APPENDS keyframe tokens with pixel-frame positions and an attention mask — a bigger change with no extra fidelity for a last frame; latent replacement is Lightricks' earlier FLF recipe and needs nothing new. `F_lat == 1` cannot hold an anchor and still generate (`error.KeyframeCanvasTooShort` → 400). Unlike the first-frame path this one is NOT graceful: the user asked for THIS ending.
