...
The same round's second row is class **D**, not A: it is unreachable off
qwen4_exp by a WRITER that only SSD-first arms, not by an arch predicate.
...
are qwen4_exp-only by construction; the third is not — it shipped gated, was
then measured on a sidecar-MTP pack, and is ungated (ae0574e).

| # | site | reach off qwen4_exp | class | gate / why not |
|---|---|---|---|---|
| 20 | `round_cost.TrialSchedule.force` re-reading its period every round | **every MTP arch and every DFlash/DSpark block drafter.** `TrialSchedule` has two consumers: `Generator.MtpWidthTrial` (aliased, driven from `mtpRoundPlanInner` for any model with a draft head, `.qwen` sidecars included) and `WidthChooser.trial` (`WidthChooser.pick`, behind the opt-in `MLX_SERVE_DFLASH_CHOOSER=1`). Neither is arch-gated | **A — measured, then ungated (ae0574e)** | `round_cost.schedulePeriodReread(layout)` is still the ONE predicate at both call sites, but it now answers **yes for every layout** — kept a predicate, not a literal `true`, so a layout can still opt out with a measurement. It shipped gated to `.long` on the argument that the re-read moves WHICH rounds of a request carry a 3-4% trial block and the 27B sidecar pack had never been measured on it. It then was: Qwen3.8-27B 4-bit (sidecar MTP, `.legacy`) on an M4 Max, `MLX_SERVE_ROUND_COST_PERSIST=0`, llmprobe decode tok/s — 4k **75.1, 75.1** ungated against **70.5 / 67.5 / 76.2** gated, 32k **52.5, 53.7** against **54.2 / 55.7 / 55.0**, i.e. both rungs inside the per-boot swing. What moves is the FLOOR, not the mean: on the tester's own trace the first trial lands at round **20** instead of **136**, and the same short requests run 4.73 tok/round ungated against 1.44 gated (26.9.1 ran 3.57). The `WidthChooser.pick` consumer inherits the re-read and is **unmeasured** — it is opt-in, and the 27B numbers are the MTP consumer's. Tests: `the trial-period re-read is EVERY layout's; the arm-once schedule below is what a93e2c0 shipped` (round_cost.zig), `a sidecar boot's width-trial SCHEDULE re-reads its period like qwen4` (generate.zig) — both still run the tester's trace shape on BOTH arms (`reread` false and true, 136 vs 20), the `false` arm surviving purely as a93e2c0's characterization; the second also scans the production call site for the layout argument. The qwen4-only gate recorded here was this round's shape; the maintainer's ungating supersedes it. Story: `docs/gotchas/engine-mlx.md`, "Then it was measured, and ungated" |
...
### Round 7 — two fixes written on main, folded into the PR (rows 24-25)

Both were authored against `main`, not against this branch, and were brought
in on the maintainer's call because the PR is what ships. Neither is a
long-context optimization, so neither takes `longCtxGated()`.

| # | site | reach off qwen4_exp | class | gate / why not |
|---|---|---|---|---|
| 24 | `transformer.ropeAngleRows` — `errdefer` → `defer` on the YaRN arm's `scaled`, plus the missing `errdefer` on `flat` in BOTH arms | **none in practice, and no arch changes behaviour.** The probe is reached from `qkAngleRowsFor`, the angle-table cache for the fused decode QK-norm+RoPE kernels; the leaking arm is `mscale != 1.0`, and qwen4_exp is the one shipped arch that both ropes with a YaRN attention factor and is hd 256 — the qwen3.5 sibling on the same kernel returns through the unscaled early-out above it. An arch that never takes the scaled arm never held the extra handle | **A — genuine bug fix, ungated** | Not a #363 defect at all: `main` leaks identically, and the fix is a cherry-pick of `9870ba5` written there. One live `mlx_array` per decode tick, each pinning the `mlx_async_eval` MTLSharedEvent stamped on it, to the 65,535 `IOSurfaceSharedEvent` cap (~82k decoded tokens, then every later eval fails with `[Event::Event] Failed to create Metal shared event` until the process is killed). `flat` is a reshape VIEW that holds its own reference, so the intermediate was always ours to drop — same shape as row 23's class. Output is byte-identical on every arch and every arm; this is ownership, not math. Guard: a settled-residency balance across the `mlx.fault` sweep, over EVERY path of the probe including `k == 0` (the un-faulted success), red-on-revert at 256 bytes. Story: `docs/gotchas/engine-mlx.md`, "One leaked `mlx_array` per decode tick wedged the process at the 65,535 MTLSharedEvent cap" |
| 25 | `--max-tokens` as the serve-mode default for requests that omit the field: `ServerConfig.default_max_tokens` / `launchMaxTokensDefault()` / `omittedMaxTokensDefaultWith`, `resolveRequestMaxTokens` on all four JSON surfaces, the file-level CLI var in `main.zig` | **every arch and every chat surface** (OpenAI chat + completions, Anthropic messages, Responses, Ollama) — but only when the operator passes the flag | **A — opt-in flag, default byte-identical** | `default_max_tokens` is 0 = unset, and at 0 `omittedMaxTokensDefaultWith` returns the `omittedMaxTokensDefault` sentinel a93e2c0 returned, pinned by a characterization on the pure core. So a boot without the flag is unchanged on every arch. With the flag it is one rung of the documented ladder (body > launch flag > `generation_config.json` > hardcoded): a request that sends a real `max_tokens` keeps it, still clamped to the context, and `0`/negatives are still omissions. It rides in because the admission bill reserves KV for the budget the request RESOLVED to, so an omitted-`max_tokens` turn on a 786k-context model books the whole remaining window (only `KVCache.RESERVE_GEN_HEADROOM` caps it) and agent clients omit the field on every turn — the seam it acts on is #363's own reservation, which is why it belongs here rather than only on main. One `/v1/messages` bug fix rides along: the hand-rolled parse `@intCast`ed a negative into a `u32`, UB in ReleaseFast. Guards: `--max-tokens is the omitted-field default`, `an omitted max_tokens reserves the LAUNCH default, not the whole window` (at the `KVCache.reservedTokens` seam), `every text surface resolves an omitted max_tokens through ONE helper`, `every serve path passes the --max-tokens default (the flag-eater class)`, `tests/test_headless_spec_flags.sh` [4/5]. Story below: "The sentinel is OURS: an omitted `max_tokens` reserves the window". The commit's CLAUDE.md one-word edit was dropped on the cherry-pick: it would have put the file at 100,005 B, over the 100,000 B cap |

### Round 8 — the qwen4_exp load-time bounds (rows 26-29)

A read of the arch's load path found four config-driven bounds whose only
guard was a `std.debug.assert`, i.e. no guard in anything we ship. All four
are qwen4_exp-only **by construction**, not by a predicate: the fields, the
types and the file they validate exist on no other arch. Story:
`docs/gotchas/models-media.md`, "A config-driven bound guarded only by a debug
assert is unguarded in every shipped binary".

| # | site | reach off qwen4_exp | class | gate / why not |
|---|---|---|---|---|
| 26 | `model.zig`'s qwen4 branch → `validateQwen4Config`; `qwen4_exp.NgramHash.init` now fallible | **none.** `ngram_size` / `heads_per_ngram` / `ngram_vocab_*` are read ONLY inside `if (model_type == "qwen4_exp")`, and `NgramHash` is constructed at exactly one production site, inside `if (config.isQwen4())` | **A — qwen4-only by construction** | `NgramHash.multipliers` is `[MAX_NGRAM_SIZE]i64`, `vocab`/`offsets` are `[MAX_HEADS]i64` and `SSMCacheEntry.ple_prev` is written `ngram_size - 1` deep; the loops write `ngram_size` and `(ngram_size-1)*heads_per_ngram` entries from the CONFIG. `std.debug.assert(h.n_heads <= MAX_HEADS and ngram_size <= 8)` was the whole guard — compiled out of every `-Doptimize=ReleaseFast` binary. Now: `2 <= ngram_size <= MAX_NGRAM_SIZE`, `heads_per_ngram >= 1`, `(ngram_size-1)*heads_per_ngram <= MAX_HEADS`, divisor and vocab base non-degenerate, refused by NAME (`InvalidQwen4NgramSize` / `…NgramHeads` / `…NgramVocab`) so the client sees `Model load failed: <name>`. A wrong-typed or negative value is `InvalidQwen4ConfigField` rather than the silently-kept DEFAULT (`if (v == .integer)`) or an unchecked `@intCast` of a negative. Tests: `qwen4_exp config: an n-gram bound past the fixed arrays is a named load error`, `… a wrong-typed or negative bound is a refusal, never a silent default`, `NgramHash.init refuses a config past its fixed arrays instead of asserting` |
| 27 | `validateQwen4Config`'s indexer arm | **none.** `indexer_*` is parsed in the qwen4 branch only, and every reader sits behind `is_qwen4 and config.indexer_n_heads > 0` | **A — qwen4-only by construction** | The QSA arm arms on `indexer_n_heads > 0` ALONE, and the forward then `@divTrunc(kv, ratio)` (illegal behaviour at ratio 0 in ReleaseFast, a trap in Debug) at ~10 sites and selects `budget / ratio` blocks — `block_topk` 0 selects nothing. Two sites had already grown local `ratio == 0` / `@max(ratio, 1)` patches, which serve a model whose block stride does not match its own checkpoint; the fix is at LOAD (`indexer_head_dim > 0`, `indexer_compress_ratio >= 1`, `indexer_budget >= indexer_compress_ratio` ⇒ `InvalidQwen4Indexer`), and the dense arm (`indexer_n_heads == 0`) still loads with every indexer field at 0. Test: `qwen4_exp config: an armed QSA indexer must carry a usable budget and ratio` |
...
### Round 9 — the refused warm turn that destroyed its own session (rows 30-32)
...
### Round 10 — three LOW observability findings (rows 33-35)
...
| # | site | reach off qwen4_exp | class | gate / why not |
|---|---|---|---|---|
| 33 | `transformer.loadQwen4Mtp`'s load line — "spec wiring pending" → "drafts armed by `--mtp`" | **none.** `loadQwen4Mtp` is reached from the `qwen4_exp` arm of `Transformer.init` alone | **A — log text only** | The wiring it called pending has been live for releases: `scheduler.zig` hands the head over as `MtpHeadRef.qwen4`, `qwen4BuildDraftRerank` runs at load, and `/v1/models` reports `mtp_loaded: true`. The one in-process proof the head exists read as "loaded but inert", which is the opposite of true. `params.mtp_enabled` is a scheduler-side `LoadParams` field and is NOT visible at load, so the line names what ARMS the head rather than claiming a state it cannot know. The marker `[qwen4] MTP head loaded` stays a PREFIX — `tests/test_mtp_equivalence.sh:264` greps it. Guard: `the qwen4 MTP head load line names the arm that runs, never a pending one` |
...
### Round 11 — the 0-token restore that still held its entry (row 36)
...
### Round 12 — the log line that accused clients of asking for a billion tokens (row 37)

One commit written against `main` and folded in on the same argument as row
25: the sentinel it prints is #363's own reservation seam. No arithmetic
moves — the number every surface resolves is unchanged, only the way two log
lines name it.

| # | site | reach off qwen4_exp | class | gate / why not |
|---|---|---|---|---|
| 37 | `server.AUTO_MAX_TOKENS_SENTINEL` (named, was a literal), the pure `MaxTokensOrigin` / `maxTokensOrigin` / `describeMaxTokens` / `maxTokensBudgetSqueezed` / `autoBudgetWindowTight`, the three request log lines (`/v1/chat/completions`, `/v1/completions`, `/v1/messages`) and the squeeze branch | **every arch and every text surface** — the request line is printed for all of them, and none is gated | **A — log text only, no behaviour** | `omittedMaxTokensDefault` still returns the same sentinel and `clampMaxTokens` still trims it the same way, so what is generated, what is reserved and what is refused are byte-identical on every arch; the only observable change is what the log calls the number. The request line now names the SOURCE (`max_tokens=auto`, `auto (4096)` for the finite unknown-context fallback, `65536 (launch default)` when row 25's `--max-tokens` filled the field, the client's bare number otherwise), and the squeeze warning stops firing for the sentinel — with no requested number there is nothing to fall short OF — while the auto case keeps its information at `debug` under its own window-relative bar. It does not take `longCtxGated()` because there is nothing to gate: a log string is not a code path, and the misreading it fixes (an external auditor read the pair as agents pooling one budget) is on every surface, not just the long-context one. Guards: `describeMaxTokens: an omitted max_tokens logs as auto, never as the sentinel`, `the max_tokens squeeze warning is silent for the auto budget, loud for a real over-ask`, plus the extended class scan `every text surface resolves an omitted max_tokens through ONE helper`, which now also pins that each surface labels its log line from the same parsed field |
...
Row **19** of the PR #363 ledger, above: class **D**, unreachable off qwen4_exp
