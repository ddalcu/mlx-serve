...
## A 400 emitted after the SSE headers is a status line in the event body

Caught in review on the SDXL branch, before it shipped, but the shape is the
one that makes it worth a rule: `handleImage` gained three new request fields
(`negative_prompt`, `guidance`/`guidance_scale`, `timestep_spacing`), two of
which can refuse, and the parsing block was written directly BELOW
`if (want_stream) try conn.writeAll(sse.headers)`. On a `"stream": true`
request that refusal writes `HTTP/1.1 400 …` and a JSON body into a socket the
client is already reading as `text/event-stream`, after a `200 OK` it has
already parsed. curl reports the FIRST status line, so the arm reads as a
clean 200 and the caller sees a truncated event stream instead of the reason.

The handler already had the correct pattern for genuinely-late failures — the
content-filter and generation-failed arms branch, `sse.sendError` when
streaming and `sendError` otherwise — so the rule is not "never fail after the
headers", it is that anything decidable from the BODY is decided above the
header write, where every other 400 in the handler already lives. Validation
that reads only the request cannot need the socket's state.

Guard: `tests/test_sdxl_gen.sh` [5] asserts each of the three refusals twice,
plain and with `"stream": true`. The streaming arm is the load-bearing one: the
plain arm passes either way.
