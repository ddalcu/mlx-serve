...
15. ~~**LTX audio-to-video (a2vid): make characters speak exact words.**~~ **LANDED 2026-07-02.** `audio` (base64 WAV) on `/v1/video/generations`: user clip → mel + audio-VAE-encoder (ported, cos 1.000000 vs the MLX reference) → tokens FROZEN through stage 1 (scalar audio timestep 0 in the DiT; exact fixed point of the guided Euler loop, pinned by weights-gated tests) → stage-2 joint refine unchanged → ORIGINAL clip muxed into the mp4 at native rate. Two-stage only (one_stage+audio → 400). App: "Speech & sound" section in the Video pane (file picker or Qwen3-TTS "Speak text" chaining), request forces two_stage + reference guidance from one-stage presets. Guards: wav.zig decode/resample tests, encoder oracle (`tests/dump_ltx_audio_encoder_fixtures.py`), a2vid tripwires in ltx_video.zig, `tests/test_video_gen.sh` a2vid case (passthrough byte-fidelity + engagement log + 400), MediaGenServiceTests request contract.
