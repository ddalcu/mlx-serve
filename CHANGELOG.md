...
- **Gemma long-context prefill more than doubled.** A custom flash-attention Metal kernel now handles Gemma's sliding-window attention during prefill, skipping everything outside each layer's attention window instead of computing and masking it. Measured on gemma-4-26B-A4B with a ~100K-token prompt: 299 → 671 tok/s prefill (2.2×) with peak memory down from 27 GB to 24.5 GB, and byte-identical output.
