...
- **Your 3D models can now move — automatic rigging, on-device.** Flip on "Rig (skeleton)" and the mesh comes back with a full animation skeleton inside: a native port of UniRig (SIGGRAPH 2025) predicts the bone structure autoregressively from the shape, geodesic skinning binds every vertex to the right bones (so a hand near a hip doesn't stick to it), and the GLB carries standard glTF skinning that animates in any engine. Validated against the PyTorch reference to near-bit-exact parity (greedy skeleton decode matches token-for-token); combines freely with the texture stage. `"rig": true` on `/v1/3d/generations`.
- **Talk to your creations: the Avatar window.** A new window in the tray turns a generated 3D model into a talking persona: give it a personality (an editable prompt, optionally grounded in a folder of your documents), a voice (clone from any short clip via Qwen3-TTS), and a face (any model from the 3D pane). Speak or type — the answer streams back sentence-by-sentence in the cloned voice while the model is still thinking, and a rigged avatar sways idly, nods or waves on command, and moves its jaw in time with the speech. Every piece runs locally.
