...
- **Agent file edits recover instead of looping.** When a smaller model fumbles an edit — sends a line range without its start line, or a line number with stray punctuation — the agent now gets a precise correction naming exactly what to add, with a real example built from its own call, and fixes itself in one retry instead of repeating the same failing edit until the turn dies.
