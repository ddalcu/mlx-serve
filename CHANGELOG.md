...
- **Document indexing runs on the GPU — about 5× faster.** Download a small embedding model from the Model Browser (e.g. `bge-small-en-v1.5-8bit`, 35 MB — encoder repos are now offered there) and folder indexing rides the server's GPU: a 500-file folder indexes in ~7 s instead of ~33 s, with your CPU left free. Without one, indexing falls back to on-device Apple embeddings, now ~25% faster from a wider worker pool. The `/v1/embeddings` API got the same treatment for everyone: input arrays embed in single batched GPU passes (~1.4 ms per 1200-char passage), results identical to one-at-a-time calls, and the encoder hot-loads beside your chat model.
