...
- **The n-gram table's long-prompt reads go through the same prefetch pool as decode.** Flash Next reads scattered rows out of a 32 GB memory-mapped table; during a long prefill those reads were still a serial walk, which on a cold table means one SSD fault at a time. They can now use the parked reader pool decode has used since 26.9.1. Opt-in with `QWEN4_PLE_PREFETCH_PREFILL=1`: on a table that is already warm the pool costs 4-5% prefill at every prompt length (M4 Max, 6/6 cells), so the serial walk stays the default and the pool is for the cold, just-evicted table it was built for. The log says which arm ran.
- **A speculative reply no longer gets stuck at the narrowest draft width for its whole length.** On a long prompt the width controller sometimes ran an entire reply one draft at a time (about 1.9 tokens per step where the same prompt reaches 3.5 to 3.9 on another boot), because the periodic width trial that would have measured the next width had been scheduled from a neighbouring context bucket's prices, over a hundred rounds out, and never rescheduled once the request's own bucket came alive. The trial schedule now follows the current bucket, so a shorter period pulls the next trial in. Applies to main as much as to this branch; the switch-off floor also moves from 8k to 32k context, where the measurements say a round starts losing to a plain step, so the serial probes stay out of shorter conversations.
