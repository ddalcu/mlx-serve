...
## v26.7.1 — LTX videos: two-stage quality, sound, and start from your own image

- **Two-stage video quality is back — and native.** The Quality and Super-Quality video presets now run the full reference two-stage pipeline on the native engine: a guided half-resolution pass on the dev model (CFG + modality guidance, with the second-order res_2s sampler for Super-Quality), a learned 2× latent upscale, then a distilled refine at full resolution. This matches the Python pipeline the engine was compared against — the upscaler port was validated at cosine 0.9998 to the reference.
- **Fast videos got their intended model.** The one-stage path now prefers the distilled transformer with its trained 8-step schedule (previously it ran the dev model unguided, which cost quality). Existing dev-only downloads keep working; the distilled variant and the upscaler are pulled automatically with the LTX bundle.
- **Your video settings actually apply now.** The CFG / STG sliders and the pipeline choice were silently dropped from requests — Quality presets ran as unguided fast generations. Fixed end to end.
- **Generated videos now actually have audio.** LTX-Video clips are produced with a synchronized stereo soundtrack: the model's audio is decoded entirely on your Mac — a mel-spectrogram VAE followed by a BigVGAN vocoder, both ported to Apple Silicon — and muxed straight into the mp4, so the file plays with sound. The clip you save and the clip you preview both carry the track, timed to the video.
- **Make your characters speak.** Put the spoken words in quotes in your video prompt — short phrases with acting directions between them — and LTX generates the voice, timed to the picture. A new "Talking character" example in the Video pane shows the format, and audio guidance on the Quality presets now steers harder toward clean speech: clearer voices, less stray background noise.
- **Audio-to-video: guaranteed words, synced lips.** Attach a real speech or music clip in the Video pane's new Speech & sound section — or type a line and have the local Qwen3-TTS voice speak it — and the video is generated *against* that soundtrack: voices, lip sync, and performance follow the clip, and the original audio (not a lossy re-synthesis) lands in the mp4. Any WAV/MP3/M4A works, the frame count auto-fits the clip length, and everything runs on-device through the same one-click LTX download (`audio` field on `/v1/video/generations` for API users).
...
- **Image-to-image: remix your own pictures.** Choose a source image in the Image pane, dial the variation strength, and both image models (FLUX.2-klein and Krea-2-Turbo) generate a prompt-guided variation of it — the picture is VAE-encoded and partially renoised entirely on-device. The needed encoders ship inside the model downloads you already have (both ports validated by encode→decode round-trips at pixel correlation 0.999+), so there's nothing extra to fetch (`image` + `strength` on `/v1/images/generations` for API users).
- **Style LoRAs for image models.** Attach any diffusers-format LoRA `.safetensors` under the Image pane's Advanced options to restyle FLUX or Krea generations. Adapters apply at runtime — no re-quantization, zero quality loss on the base weights — and detach cleanly between requests (`lora_path` / `lora_scale` on the API).
...
- **Comes with the model, automatically.** Both the audio decoder and the image encoder ship in the standard LTX download, so sound and image-to-video just work with no extra step or separate fetch; a checkpoint without them still generates and plays with no error.
...
- **The sandbox is online — with live port mapping.** The agent's Linux sandbox now has real internet access (DHCP handled by the kernel itself, so it works with any base image), and any server the agent starts inside it is instantly reachable on your Mac: an Express app on port 8080 appears at localhost:8080, a dev server on 3000 at localhost:3000 — mapped live as ports open and close, bound to localhost only. The tray now also shows the sandbox's live RAM usage, and a Settings toggle turns networking off for a fully isolated guest.
