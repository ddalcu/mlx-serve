...
- **Speculative verification on Flash Next reads long contexts through one fused kernel.** Checking a batch of draft tokens against a long prompt used to gather the selected key blocks into a padded scratch buffer and run ordinary attention over it. That work is now a single kernel that reads the packed keys and values in place, split across the key range so the GPU has something to do on every core — the first version gave each query row one thread group, which left 12 of 40 cores busy and ran SLOWER than the gather it replaced. At 62k tokens on an M5 Max a verify forward is about 13% faster at width 6 and 8% at width 3. The kernel serves a quantized cache (`--kv-quant 4|8`) only: with the default `--kv-quant off` the gather is a plain copy and the fused kernel ran 4-8% slower than it on an M4 Max, so the dense cache keeps the gather. `MLX_SERVE_QSA_ATTN_KERNEL=0` restores the gather on a quantized cache; `MLX_SERVE_QSA_ATTN_NSPLIT` (default 16) and `MLX_SERVE_QSA_ATTN_MIN_S` (default 2) set the split count and the lowest width the kernel serves.
...
- **The n-gram table's long-prompt reads go through the same prefetch pool as decode, once the prompt is long enough to need it.** Flash Next reads scattered rows out of a 32 GB memory-mapped table; during a long prefill those reads were still a serial walk, which on an evicted table means one SSD fault at a time. They now use the parked reader pool decode has used since 26.9.1 — but only past 64k tokens of context. Below that the table is still resident and the pool's wake-up cost makes prefill 5-8% slower (measured on two machines, every cell); above it the serial walk is the one that costs, up to four times as much per thousand tokens on a 374k prompt. `QWEN4_PLE_PREFETCH_PREFILL_MIN_KV` moves the threshold, `QWEN4_PLE_PREFETCH_PREFILL=0` or `=1` forces one arm, and the log says which arm ran.
- **A speculative reply on Flash Next no longer gets stuck at the narrowest draft width for its whole length.** On a long prompt the width controller sometimes ran an entire reply one draft at a time (about 1.9 tokens per step where the same prompt reaches 3.5 to 3.9 on another boot), because the periodic width trial that would have measured the next width had been scheduled from a neighbouring context bucket's prices, over a hundred rounds out, and never rescheduled once the request's own bucket came alive. The trial schedule now follows the current bucket, so a shorter period pulls the next trial in. This reaches every model with a draft head: on Qwen 3.8 27B the same schedule left short requests one draft at a time (1.4 tokens per step where 26.9.1 ran 3.6), and the re-read brings a 4k prompt from 67 to 75 tok/s on an M4 Max. The point past which speculation switches itself off for a plain decode step also moves from 8k to 32k of context, where the measurements say a round starts losing, so its probes stay out of shorter conversations.
...
### Fixes
- A cached conversation on Flash Next could be protected from eviction by a request it gave nothing to. When a long conversation is handed to a request, the cache notes which entry that was so the memory pass cannot throw away something the request is already holding. Two ways of missing — a restore that arrives without the sparse-attention index it needs, and one with no usable restore point below the match — set that note first and then returned nothing, having already handed every byte back: the entry was shielded from the very pass trying to admit the request its own miss had just sent to a full re-prefill, and it was also promoted to most-recently-used for a turn it did not serve. Both are now undone when the turn ends up with nothing. The rule that refuses a restore holding a whole session hostage for a handful of rows was reading the wrong number with it: on these conversations the rows really handed over are the ones at the nearest restore point, which can be a few hundred times fewer than the tokens matched, so a share that looked proportionate could pin an entire 11 GB session to deliver eight rows. It now weighs what the restore delivers, and says so when it declines. Flash Next only, and only where the SSD-backed cache is on.
- `mlx-serve launch claude` drove a long-context server as a 200k one. Claude Code assumes a 200k window for any model outside its own catalog and auto-compacts to it, which on a 786k-context server threw away three quarters of the conversation the server was willing to hold. The launch script now exports `CLAUDE_CODE_MAX_CONTEXT_TOKENS` set to the context the server advertises on `/v1/models`, the same number the launcher already writes into every other agent CLI's config, and the same one the app's Claude Code setup instructions and launcher print. A server that advertises no context is left alone rather than pinned to a made-up number.
- A long conversation on Flash Next could be destroyed by a single refused turn. When a cached conversation is handed to a request, the cache marks it as that request's and stops offering it to anyone else. If the request was then refused before it ran — the memory check does this, by design, when a prompt does not fit — the conversation was thrown away with it: on a live 364k-token agent session, 7.8 GB and the whole prefix, for a turn that never ran a single forward. The next turn had nothing to restore and could not be prefilled from scratch either, so the session ended. The handover is now split in two: the cache marks the entry when the request is admitted, but only gives up the data immediately before the first token is written, which is after every check that can refuse. A request that ends before that point hands the conversation back untouched — same tokens, same state, same position in the cache — and only a request that really started writing takes it away. Flash Next only, and only where the SSD-backed cache is on.
- The refusal that goes with it named the wrong number. It reported the whole resident cache as if it had been evicted ("~13374MB available after evicting ~7802MB of hot cache") when the only entry was the request's own conversation, which eviction refuses to take by construction — the same request's admission line, one screen earlier, had already reported nothing reclaimable. The refusal now quotes the comparison it actually made and reports the resident bytes as context, naming how much of them the request itself pins.
- A warm turn on Flash Next was billed for recurrent-state checkpoints it does not take. The memory estimate charged every request for the maximum number of checkpoints a prompt of that length would produce, but a turn that restores a cached conversation only processes the new text at the end of it — a few thousand tokens, one checkpoint — while the ones already in the cache are the entry's own and are merged, not allocated. On the live 364k session that was 1.3 GB of a bill that missed by 0.3 GB: the turn is now served. Cold prompts are billed exactly as before, and other architectures are unaffected.
- A malformed Qwen 3.8 Flash Next checkpoint could be loaded instead of refused. Four values that the model's own `config.json` and its 32 GB n-gram table declare — how many tokens the n-gram hash mixes, how many hash heads each of them gets, the sparse-attention block size and budget, and which layer the n-gram embedding is injected at — sized fixed arrays, divided, or placed a layer with no check that survives an optimized build: the one guard present was a debug assertion, which the binaries we ship compile out. A pack claiming twice the supported number of hash heads wrote past the end of two arrays before the first token; one with a zero block size divided by zero; one naming a layer that does not exist loaded a model with the n-gram term silently missing altogether, which produces plausible-looking text rather than an error; and the table's header was trusted enough that a truncated or hand-edited one could be read past the end of the mapped file. All four are now checked when the model loads and refused by name, so the error says which field is wrong (`Model load failed: InvalidQwen4NgramHeads`) instead of failing later or not at all. A wrong-typed or negative value in those fields is a refusal too, rather than silently keeping the default. The shipped packs are unaffected — they load exactly as before — and no other architecture has these fields.
...
- Long Flash Next sessions eventually wedged: after roughly a day of steady decoding every request failed with "Failed to create Metal shared event" and only a restart fixed it. One small array per decoded step was never released, and each one held on to a scarce system handle until the 65,535 available ran out. Other models were unaffected.
- `--max-tokens N` now sets the reply budget for clients that do not ask for one. An OpenAI-style client that omits `max_tokens` means "generate until you are done, bounded by the context window", and that is still what it gets by default — but the admission check has to reserve room for a reply that large, so on a long-context model one such request could hold enough of the machine to keep a second one out. Coding agents omit the field (opencode does on every turn). `--max-tokens` previously applied only to offline `--prompt` runs; in `--serve` mode it is now the default for a request that omits the field or sends `0`, on every chat surface (OpenAI chat and completions, Anthropic messages, Responses, Ollama). A request that sends a real `max_tokens` is unaffected, the value is still trimmed to what is left of the context, and with no flag nothing changes. Set to taste, e.g. `--max-tokens 65536`; the resolved value is written to the log at startup.
...
- Flash Next's startup line for its draft head said the speculative wiring was still pending. It has not been for several releases — the head is handed to the scheduler at load, the draft shortlist is built with it, and the model list already reports it as loaded — so the one message confirming the head exists read as though it were inert. The line now says the head is loaded and that `--mtp` arms it, which is what actually decides whether it drafts. The `[qwen4] MTP head loaded` prefix is unchanged for anything grepping the log.
- Flash Next asked the operating system for the same four debug switches on every attention layer of every step, about a dozen times per generated token, for answers that cannot change while the server runs. They are read once now. Nothing about the switches changes: the ones that must treat `=0` as off still do, so a test harness exporting `QWEN4_PROFILE_FWD=0` still cannot turn a profiler on.
- Flash Next's background warm-up of its 51 GB n-gram table now says what it is doing while it does it. That warm-up is the difference between a first long prompt taking about a minute and taking about three, and its only sign of life was a single line roughly three minutes in, after it finished — with `MLX_SERVE_NGRAM_WARM=0` it said nothing at all. It now logs when it starts, roughly every 8 GB or 10 seconds while it runs, and once when it is done; the disabled case says so and warns that the first long prompt will read the table off the SSD. Progress is also readable without the log: `/props` carries a `ngram_warm` object with bytes read and total, and `--metrics` exposes `mlx_serve:ngram_warm_bytes`. Both are absent or zero on every other model.
- The server log accused clients of asking for a billion tokens. When a request leaves `max_tokens` out — which is most agent clients, and which means "generate until you are done, inside the context window" — the server fills in a placeholder of 1,073,741,823 and then trims it to whatever the window has left. Two log lines printed that placeholder as though the client had sent it: the request line (`max_tokens=1073741823`) and a warning on every such request past a quarter of the window (`generation budget squeezed: 418224/1073741823 tokens remaining`), which one reader took as evidence that several agents were pooling one giant budget. The request line now says `max_tokens=auto`, or `max_tokens=65536 (launch default)` when `--max-tokens` filled the field, and prints a client's own number only when the client sent one. The squeeze warning is again what it was meant to be — a heads-up that a budget somebody actually asked for will not fit, so tool-call arguments may be truncated — and no longer fires for the automatic one; when the prompt has eaten three quarters of the window the automatic case says so at debug level instead. Nothing about how much is generated changes.
