...
- Raising `iogpu.wired_limit_mb` now actually raises what a long-context request may plan for. On a 128 GB Mac with the limit at 120 GB, a 448k-token conversation was refused an append it was 516 MB short of, because the guard sized itself against how much RAM other applications happened to leave free (about 100 GB) rather than the limit the operator set. On Flash Next the raised limit is now a floor, 8 GB under it — below the deepest allocation that box has actually completed. Every other model is unchanged.
- A long conversation was billed for its whole cached prompt again on every turn that extended it, so a 448k-token session was refused an append of 73 tokens. The cache is grown one layer at a time and each old buffer is released as the prefill moves on, so a warm turn is now billed what it actually allocates: the same turn goes from 13,450 MB to 7,051 MB, and it no longer falls back to the narrowest prefill width. Flash Next only.
