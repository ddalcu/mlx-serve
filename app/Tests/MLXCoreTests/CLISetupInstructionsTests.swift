...
final class CLISetupInstructionsTests: XCTestCase {
    ...
    func testCodexTabWritesTheProfileIntoTheCodexHome() throws {
        ...
        XCTAssertTrue(tab.command.contains(#"cat > "$PROFILE" <<EOF"#), tab.command)
        // an existing profile is merged in place (awk), not clobbered
        XCTAssertTrue(tab.command.contains("if [ -f \"$PROFILE\" ]; then"), tab.command)
    ...
    /// The builder rewrites ONLY the mlx-serve keys in place; the user's own
    /// settings, foreign tables, and comments all survive the merge.
    func testCodexConfigRewritesManagedKeysAndKeepsEverythingElse() throws {
        let fresh = AgentConfigs.codexConfigTOML(baseURL: "http://x:1", model: "m1", budget: budget)
        XCTAssertTrue(fresh.hasPrefix("# Generated by mlx-serve. On each launch only the mlx-serve keys below are\n# rewritten in place; your own settings, tables, and comments survive.\n"), fresh)
        ...
        XCTAssertFalse(merged.contains("http://x:1"), merged)
        // the user's own lines ride verbatim
        XCTAssertTrue(merged.contains("# my own review policy"), merged)
        XCTAssertTrue(merged.contains("approval_policy = \"on-request\""), merged)
        // a missing managed key is re-added at its scope
        XCTAssertTrue(merged.contains("model_context_window = \(budget.context)"), merged)
        XCTAssertTrue(merged.contains("name = \"MLX Serve (local)\""), merged)
        XCTAssertEqual(merged.components(separatedBy: "[projects.\"").count - 1, 1)

        // Foreign tables are the user's own config territory and must survive.
        let foreign = existing + "\n\n[mcp_servers.thing]\ncommand = \"x\"\n"
        XCTAssertTrue(AgentConfigs.codexConfigTOML(baseURL: "http://x:2", model: "m2", budget: budget,
                                                   existing: foreign).contains("[mcp_servers.thing]\ncommand = \"x\""))
    }

    /// No `[model_providers.mlx]` at all: the whole table is created at the
    /// tail, after the trust table, so it still parses as its own table.
    func testCodexConfigCreatesTheMlxProviderTableWhenAbsent() throws {
        let existing = """
        model = "old"
        model_provider = "mlx"
        ...
        """
        let merged = AgentConfigs.codexConfigTOML(baseURL: "http://x:2", model: "m2", budget: budget, existing: existing)
        XCTAssertTrue(merged.contains("[model_providers.mlx]\nname = \"MLX Serve (local)\"\nbase_url = \"http://x:2/v1\"\nwire_api = \"responses\""), merged)
        XCTAssertTrue(merged.contains("[projects.\"/work/a\"]\ntrust_level = \"trusted\""), merged)
        XCTAssertTrue(merged.contains("model = \"m2\""), merged)
        let mlxAt = merged.range(of: "[model_providers.mlx]")!.lowerBound
        let projAt = merged.range(of: "[projects.")!.lowerBound
        XCTAssertGreaterThan(mlxAt, projAt)
    }

    /// The tab's embedded awk merge block must behave like the Swift builder
    /// it mirrors: run the awk program extracted from the tab's own command
    /// and compare its output against `codexConfigTOML(existing:)` on the
    /// same inputs. A paste-able instruction script that drifts from the
    /// launcher silently strands the user's profile on stale settings.
    func testCodexTabAwkMergeMatchesTheSwiftBuilder() throws {
        let tab = try XCTUnwrap(tabs.first { $0.id == "codex" })
        let lines = tab.command.components(separatedBy: "\n")
        guard let start = lines.firstIndex(where: { $0.contains("awk -v model=") }),
              let end = lines.firstIndex(where: { $0.trimmingCharacters(in: .whitespaces).hasPrefix(#"' "$PROFILE""#) }), end > start
        else { return XCTFail("no awk merge block in the codex tab") }
        let program = lines[(start + 1)..<end].joined(separator: "\n")

        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("codex-awk-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
        let progPath = dir.appendingPathComponent("merge.awk")
        try program.write(to: progPath, atomically: true, encoding: .utf8)
        ...
        trust_level = "trusted"
        """
        let profilePath = dir.appendingPathComponent("mlx-serve.config.toml")
        try existing.write(to: profilePath, atomically: true, encoding: .utf8)

        let process = Process()
        process.executableURL = URL(fileURLWithPath: "/bin/sh")
        process.arguments = ["-c",
            "awk -v model=\"$1\" -v ctx=\"$2\" -v url=\"$3\" -f \"$4\" \"$5\" > \"$5.new\" && mv \"$5.new\" \"$5\"",
            "sh", "m2", String(budget.context), "http://x:2/v1", progPath.path, profilePath.path]
        let out = Pipe()
        ...
        let mergedAwk = try String(contentsOf: profilePath, encoding: .utf8)
        let mergedSwift = AgentConfigs.codexConfigTOML(baseURL: "http://x:2", model: "m2",
                                                       budget: budget, existing: existing)
        XCTAssertEqual(mergedAwk, mergedSwift, "awk block drifted from the Swift builder")
    }
