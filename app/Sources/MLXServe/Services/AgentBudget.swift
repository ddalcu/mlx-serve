...
enum AgentConfigs {
    ...
    /// comment, table header, or blank). Mirrors launch.zig `tomlKeyOf`.
    static func tomlKeyOf(_ line: String) -> String? {
        let s = line.trimmingCharacters(in: CharacterSet(charactersIn: " \t"))
        if s.isEmpty || s.hasPrefix("#") || s.hasPrefix("[") { return nil }
        guard let eq = s.firstIndex(of: "=") else { return nil }
        return String(s[s.startIndex..<eq]).trimmingCharacters(in: CharacterSet(charactersIn: " \t"))
    }

    /// The table a `[name]` header opens, when the line is one; `[[name]]`
    /// array headers are declined. Mirrors launch.zig `tomlTableOf`.
    static func tomlTableOf(_ line: String) -> String? {
        let s = line.trimmingCharacters(in: CharacterSet(charactersIn: " \t"))
        guard s.count >= 2, s.hasPrefix("["), !s.hasPrefix("[[") else { return nil }
        guard let close = s.firstIndex(of: "]") else { return nil }
        return String(s[s.index(after: s.startIndex)..<close]).trimmingCharacters(in: CharacterSet(charactersIn: " \t"))
    }

    private static let codexProfileHeader = """
    ...
    /// codex `mlx-serve.config.toml` — the `--profile mlx-serve` layer,
    /// written into the effective Codex home (`${CODEX_HOME:-$HOME/.codex}`;
    /// the dir is created if missing) and merged over the user's own
    /// `config.toml` at launch. Never written to `config.toml` itself.
    ...
    /// anyway.
    ///
    /// `existing` is the file already on disk. When it exists the managed keys
    /// (`model`, `model_provider`, `model_context_window` at the root and
    /// `name`, `base_url`, `wire_api` under `[model_providers.mlx]`) are
    /// rewritten IN PLACE at their current position, and every other line —
    /// codex's `[projects.*]` trust tables, the user's own settings, foreign
    /// tables, comments — is carried over untouched. A missing managed key is
    /// re-added (root keys before the first table header, mlx keys at the end
    /// of the mlx body, or the whole table at the tail if absent). Line surgery,
    /// not a TOML round-trip: a parser would drop the comments and formatting
    /// the user sees. Mirrors launch.zig `codexConfigToml`.
    static func codexConfigTOML(baseURL: String, model: String,
                                budget: AgentBudget.Budget,
                                existing: String = "") -> String {
        let topModel = "model = \"\(model)\""
        let topProvider = "model_provider = \"mlx\""
        let topCtx = "model_context_window = \(budget.context)"
        let mlxName = "name = \"MLX Serve (local)\""
        let mlxURL = "base_url = \"\(baseURL)/v1\""
        let mlxWire = "wire_api = \"responses\""

        if existing.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return "\(codexProfileHeader)\n\(topModel)\n\(topProvider)\n\(topCtx)\n\n[model_providers.mlx]\n\(mlxName)\n\(mlxURL)\n\(mlxWire)\n"
        }

        let lines = existing.components(separatedBy: "\n")
        var firstHeader = lines.count
        var mlxHdr: Int? = nil
        var mlxBodyEnd = lines.count
        for (i, ln) in lines.enumerated() {
            guard let tbl = tomlTableOf(ln) else { continue }
            if i < firstHeader { firstHeader = i }
            if tbl == "model_providers.mlx" { mlxHdr = i }
        }
        if let h = mlxHdr {
            for i in (h + 1)..<lines.count {
                if tomlTableOf(lines[i]) != nil { mlxBodyEnd = i; break }
            }
        }

        var repl = [String?](repeating: nil, count: lines.count)
        var seen = [Bool](repeating: false, count: 6) // model, provider, ctx, name, url, wire
        for (i, ln) in lines.enumerated() {
            guard let key = tomlKeyOf(ln) else { continue }
            if i < firstHeader {
                if key == "model" { repl[i] = topModel; seen[0] = true }
                if key == "model_provider" { repl[i] = topProvider; seen[1] = true }
                if key == "model_context_window" { repl[i] = topCtx; seen[2] = true }
            } else if let h = mlxHdr, i > h, i < mlxBodyEnd {
                if key == "name" { repl[i] = mlxName; seen[3] = true }
                if key == "base_url" { repl[i] = mlxURL; seen[4] = true }
                if key == "wire_api" { repl[i] = mlxWire; seen[5] = true }
            }
        }

        var out: [String] = []
        for (i, ln) in lines.enumerated() {
            if i == firstHeader && firstHeader < lines.count {
                if !seen[0] { out.append(topModel) }
                if !seen[1] { out.append(topProvider) }
                if !seen[2] { out.append(topCtx) }
            }
            out.append(repl[i] ?? ln)
            if mlxHdr != nil && i == mlxBodyEnd - 1 {
                if !seen[3] { out.append(mlxName) }
                if !seen[4] { out.append(mlxURL) }
                if !seen[5] { out.append(mlxWire) }
            }
        }
        ...
        if mlxHdr == nil {
            out.append("")
            out.append("[model_providers.mlx]")
            out.append(mlxName)
            out.append(mlxURL)
            out.append(mlxWire)
        }
