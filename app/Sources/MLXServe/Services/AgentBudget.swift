...
enum AgentConfigs {
    ...
    /// True for the top-level tables codex itself writes back into the
    /// profile file — `[projects."<cwd>"]` holds `trust_level`, which codex
    /// persists wherever it last saw a trust prompt answered, profile layer
    /// included. Mirrors launch.zig `isCodexOwnedTable`.
    static func isCodexOwnedTable(_ header: String) -> Bool {
        let after = header.dropFirst().drop(while: { $0 == " " || $0 == "\t" })
        return after.hasPrefix("projects.")
    }

    /// The `[projects.*]` trust tables codex wrote into the profile file
    /// already on disk (`""` when there are none), in declaration order: the
    /// header line plus its body lines, up to the next table header. A table
    /// whose body is empty is a stub, not a trust entry — dropped on both
    /// sides, matching launch.zig.
    static func codexOwnedTables(_ existing: String) -> [String] {
        let lines = existing.components(separatedBy: "\n")
        var tables: [String] = []
        var index = 0
        while index < lines.count {
            let line = lines[index]
            if line.hasPrefix("[") && isCodexOwnedTable(line) {
                var body: [String] = [line]
                index += 1
                while index < lines.count {
                    let next = lines[index]
                    if next.hasPrefix("[") { break }
                    body.append(String(next.trimmingCharacters(in: CharacterSet(charactersIn: "\r\n"))))
                    index += 1
                }
                while body.count > 1, body.last?.isEmpty == true { body.removeLast() }
                if body.count > 1 { tables.append(body.joined(separator: "\n")) }
            } else {
                index += 1
            }
        }
    ...
    /// anyway.
    ///
    /// `existing` is the file already on disk: the generated keys regenerate
    /// verbatim, but codex's OWN `[projects.*]` trust tables are carried over
    /// at the tail — codex persists `trust_level` into this profile layer,
    /// and dropping them would make it re-ask directory trust on every
    /// launch.
    static func codexConfigTOML(baseURL: String, model: String,
        ...
        wire_api = "responses"
        """
        let owned = codexOwnedTables(existing)
        guard !owned.isEmpty else { return header }
        return header + "\n" + owned.map { "\n" + $0 }.joined()
    }
