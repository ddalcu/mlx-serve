...
enum AgentPrompt {
    ...
    ///
    /// `desktop` adds the sandbox desktop line (the `computer` tool) — the
    /// SETTING, not the install state, so the prefix stays stable across a
    /// session (the tool itself refuses by name while installing). It only
    /// means something inside the sandbox, so the host arm ignores it.
    static func executionEnvironmentSection(sandboxed: Bool, desktop: Bool = false) -> String {
        if sandboxed {
            return "\n\n# Execution environment\n"
                ...
                + (desktop ? " " + SandboxDesktop.promptLine : "")
        }
        return "\n\n# Execution environment\n"
    ...
    [
      ...
      {"type":"function","function":{"name":"computer","description":"Operate the Linux desktop in the sandbox (XFCE, 1280x800, Firefox installed). Loop: look (read or observe), act, then look again using the observe every action returns. Actions: research (query + sites N, up to 40: searches, visits the top N result sites in Firefox one by one and returns a digest with title, URL and excerpt per site; ONE call does the whole research, then you summarize), navigate (url, or query words to web-search; opens Firefox and returns the page as text with numbered links), read (the focused window as text: headings, paragraphs, [id] links and buttons; use it to READ a page or document), observe (every element of the focused window as [id] role \"name\" (x,y,w,h); window:\"all\" for every window), click / double_click / right_click (id, or x+y), type (text, into the focused field), key (keys like ctrl+l, Return, alt+F4), scroll (direction up|down, amount, optional id), drag (from + to ids), open (command, launches an app: xfce4-terminal, thunar, firefox-esr, libreoffice --calc; a spreadsheet is filled with ONE type call per row, e.g. text \"Apple\\t1.20\\n\": Tab moves to the next cell, newline to the next row; a plain type stays in the same cell), screenshot (ONLY when read/observe lack what you need; refused on a text-only model). Research recipe: research with the query and how many sites the user wants, then answer from the digest. After research returns, answer the user; do not call other tools. Never invent or guess URLs. Install recipe: shell `apt-get install -y <pkg>`, then computer open. Example: {\"action\": \"navigate\", \"query\": \"raspberry pi 5 price\"}","parameters":{"type":"object","properties":{"action":{"type":"string","description":"research, navigate, read, observe, click, double_click, right_click, type, key, scroll, drag, open, or screenshot"},"url":{"type":"string","description":"Web address to open (navigate action)"},"query":{"type":"string","description":"What to research or web-search (research and navigate actions)"},"sites":{"type":"string","description":"How many result sites to visit, 1 to 40, default 3 (research action)"},"id":{"type":"string","description":"Element id from the last read/observe (click, double_click, right_click, scroll)"},"x":{"type":"string","description":"Screen x when no id is known (with y)"},"y":{"type":"string","description":"Screen y when no id is known (with x)"},"text":{"type":"string","description":"Text to type (type action)"},"keys":{"type":"string","description":"Key or chord to press, xdotool names: ctrl+l, Return, Tab, alt+F4 (key action)"},"direction":{"type":"string","description":"up or down (scroll action)"},"amount":{"type":"string","description":"Scroll clicks, default 3 (scroll action)"},"from":{"type":"string","description":"Drag source id (drag action)"},"to":{"type":"string","description":"Drag destination id (drag action)"},"window":{"type":"string","description":"observe scope: focused (default) or all"},"command":{"type":"string","description":"Program to launch in the desktop session (open action), e.g. xfce4-terminal, libreoffice --calc, chocolate-doom"}},"required":["action"]}}},
