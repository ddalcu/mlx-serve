...
enum AgentPrompt {
    ...
    [
      ...
      {"type":"function","function":{"name":"browse","description":"Browse and interact with web pages. Actions: 'navigate' (load URL), 'readText' (get visible text), 'readHTML' (get raw HTML), 'click' (click element by CSS selector), 'executeJS' (run JavaScript), 'screenshot' (capture page as JPEG image). To discover interactive elements, use executeJS with: JSON.stringify(Array.from(document.querySelectorAll('input,textarea,button')).slice(0,15).map(e=>({tag:e.tagName,id:e.id,cls:e.className,ph:e.placeholder,text:(e.textContent||'').substring(0,30)})))","parameters":{"type":"object","properties":{"action":{"type":"string","description":"navigate, readText, readHTML, click, executeJS, or screenshot"},"url":{"type":"string","description":"URL to browse (required for navigate, optional for others)"},"selector":{"type":"string","description":"CSS selector for click action (e.g. 'button.submit', '#send-btn')"},"script":{"type":"string","description":"JavaScript code for executeJS action"}},"required":["action"]}}},
