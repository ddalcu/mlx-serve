...
extension ServerOptions {
    ...
    static let serverFlagFields: [String: ServerOptionField] = [
        "host": .init(
            ...
            explainer: String(localized: "Bind address. 0.0.0.0 lets other devices on your network reach the server; 127.0.0.1 is local-only. The app itself connects via 127.0.0.1, so pick an address that includes localhost."),
            needsRestart: true),
        "port": .init(
            title: String(localized: "Port"),
            explainer: String(localized: "HTTP port the server listens on (default 11234). Change if it conflicts with another local service."),
            needsRestart: true),
        "ctxSize": .init(
            ...
            needsRestart: true),
        "noVision": .init(
            title: String(localized: "Disable vision"),
            explainer: String(localized: "Skip loading the SigLIP image encoder. Saves ~3 GB of memory on text-only workloads."),
            needsRestart: true),
        "logLevel": .init(
            title: String(localized: "Log level"),
            explainer: String(localized: "Server log verbosity. Use 'debug' to capture Jinja errors, KV cache hits and per-request token counts."),
            needsRestart: true),
        "logToFile": .init(
            title: String(localized: "Log to file"),
            explainer: String(localized: "Write the server log to ~/.mlx-serve/logs/mlx-serve-<port>.log (32 MB rotation, one file per port). This is the file to check after a crash — the in-app log viewer only holds what arrived while the app was running."),
            needsRestart: true),
        "requestTimeout": .init(
            title: String(localized: "Request timeout (s)"),
            explainer: String(localized: "Max seconds a single HTTP request is allowed to take. 0 = unlimited. Long agent loops may need 600+."),
            needsRestart: true),
        "enableMetrics": .init(
            title: String(localized: "Metrics panel"),
            explainer: String(localized: "Expose Prometheus metrics at /metrics and a live throughput/latency/GPU/memory panel on the server index page. Zero cost when off."),
            needsRestart: true),
        "apiKey": .init(
            title: String(localized: "API key"),
            explainer: String(localized: "Require this key for requests from OTHER machines (the OpenAI/Anthropic/Ollama APIs + index page + metrics). Sent as Authorization: Bearer, x-api-key, HTTP Basic, or ?api_key=. Localhost is trusted, so this app never needs it — it protects the server when exposed on the network. Blank = no auth."),
            needsRestart: true),
        "lanShareEnabled": .init(
            title: String(localized: "Share my models on this network"),
            explainer: String(localized: "Advertise this Mac over Bonjour and let other Macs on your local network run inference on the models you share below. Only inference is exposed — model management, metrics, and the status page stay private to this Mac."),
            needsRestart: true),
        "lanShareAll": .init(
            title: String(localized: "Share all models"),
            explainer: String(localized: "Share everything in your library (chat AND image/audio/video/3D). Turn off to pick individual models."),
            needsRestart: true),
        "lanName": .init(
            title: String(localized: "Shown to others as"),
            explainer: String(localized: "The name other Macs see next to your shared models. Blank = this Mac's hostname."),
            needsRestart: true),
        "lanDiscoverEnabled": .init(
            title: String(localized: "Use models shared by other Macs"),
            explainer: String(localized: "Discover models other mlx-serve hosts share on this network. They appear in the model pickers as \u{201C}model · peer\u{201D} and run on the hosting Mac — nothing to download."),
            needsRestart: true),
        "toolAutocorrect": .init(
            title: String(localized: "Tool-call auto-correct"),
            explainer: String(localized: "Coerce tool-call arguments to the types the tool schema declares (e.g. a model sending Python's \"False\" or a list-as-string) so strict clients like Claude Code stop rejecting them with \"expected boolean, provided string\". Recommended on. Turn off to pass the model's arguments through exactly as written."),
            needsRestart: true),
        "enableMTP": .init(
            title: String(localized: "Multi-Token Prediction (recommended)"),
            explainer: String(localized: "Qwen 3.5 / 3.6 models that ship a trained MTP head guess several of the next tokens at once and check them all in a single pass — typically a big speed-up on replies, with output identical to normal decoding. It switches on by itself for models that have the head, and does nothing for models that don't, so there's rarely a reason to turn it off."),
            needsRestart: true),
        "mtpDepth": .init(
            title: String(localized: "Tokens guessed ahead"),
            explainer: String(localized: "How many tokens the MTP head guesses per step. Automatic (recommended) tunes this live — it guesses deeper while the model keeps accepting the guesses and backs off when it doesn't. Pick a fixed number only if you're measuring performance."),
            needsRestart: true),
        "forceMTPOnMoE": .init(
            title: String(localized: "Also use MTP on mixture-of-experts models"),
            explainer: String(localized: "Mixture-of-experts models (e.g. Qwen3.6 35B-A3B) leave their MTP head switched off by default, because checking several guessed tokens at once makes them re-route every expert and that can cost more than it saves. Some MoE heads are good enough to win anyway. Turn this on to use it — and measure: if replies get slower, turn it back off. Models without an MoE layout are unaffected."),
            needsRestart: true),
        "enableDSpark": .init(
            title: String(localized: "DSpark draft stages (DeepSeek‑V4)"),
            explainer: String(localized: "DeepSeek‑V4‑Flash ships its own 3‑stage speculative draft (DSpark). Enabling it loads about 11 GB of extra draft weights at startup, so it stays off unless you turn it on — and the server still refuses when the Mac doesn't have the memory for model + draft + working room, serving normally instead. For DeepSeek‑V4 GGUF files this arms the embedded ds4 engine's DSpark runtime instead, using the DSpark support GGUF downloaded beside the model (nothing happens without that file). Only affects DeepSeek‑V4 models; greedy (temperature 0) requests only."),
            needsRestart: true),
        "enablePLD": .init(
            title: String(localized: "Enable PLD (recommended)"),
            explainer: String(localized: "Prompt Lookup Decoding. Big wins on echo-heavy workloads (code editing, RAG, agent loops). The adaptive prompt-time gate auto-disables it on novel content. On models with a native MTP head, MTP takes priority and PLD stays dormant — except MoE models (e.g. 35B-A3B), where PLD is the default speedup."),
            needsRestart: true),
        "pldDraftLen": .init(
            title: String(localized: "PLD draft length"),
            explainer: String(localized: "Maximum draft tokens proposed per PLD step (default 5). Higher = bigger speedup when matches hit, more wasted work when they miss."),
            needsRestart: true),
        "pldKeyLen": .init(
            title: String(localized: "PLD key length"),
            explainer: String(localized: "N-gram match key length for PLD lookup (default 3). Shorter keys = more matches, lower precision."),
            needsRestart: true),
        "drafterPath": .init(
            title: String(localized: "Drafter checkpoint"),
            explainer: String(localized: "Path to a Gemma 4 assistant drafter directory (gemma-4-*-it-assistant-bf16). Must pair with a Gemma 4 target. Empty = no drafter."),
            needsRestart: true),
        "draftBlockSize": .init(
            title: String(localized: "Drafter block size"),
            explainer: String(localized: "Tokens per drafter round (default 4 = 3 drafter steps + 1 verify token)."),
            needsRestart: true),
        "maxConcurrent": .init(
            title: String(localized: "Concurrent requests"),
            explainer: String(localized: "Continuous batching: how many chat requests share one forward pass. 1 = serial. 2 is a good default for dense models (~1.5× throughput, ~33% per-request latency cost). MoE and hybrid SSM models stay serial regardless."),
            needsRestart: true),
        "decodeAttnQuant": .init(
            title: String(localized: "Fast decode for bf16-attention models (recommended)"),
            explainer: String(localized: "Serves decode from quantized copies of attention weights that ship un-quantized (bf16): 8-bit for most layers, 4-bit for the last fifth, where quantization error matters far less. Models like poolside Laguna decode ~25% faster overall. Slightly lossy: replies can differ in wording from the exact bf16 decode, but measured answers stay the same quality (prompt reading and speculative checks keep the exact weights). Flipping this ON (rather than leaving it untouched) also enables DeepSeek-V4's compressor-input requant — ~7% faster decode there, with rare wording-level artifacts. Models with fully quantized attention are unaffected. Turn off for bit-exact bf16 decoding."),
            needsRestart: true),
        "kvQuant": .init(
            title: String(localized: "KV cache quantization"),
            explainer: String(localized: "A memory-for-speed trade, not a free upgrade: shrinks KV-cache RAM (8-bit ≈ 2× smaller, 4-bit ≈ 4×) but makes decode ~10% slower at typical contexts — and slower still on long ones, since every generated token pays a dequantize step. Turn on when memory is the constraint (long contexts or big models on a 16 GB Mac); leave OFF for maximum tokens/sec if you have plenty of RAM. TurboQuant variants add a per-layer Hadamard rotation for heavy-tailed activations."),
            needsRestart: true),
        "prefixCacheEntries": .init(
            title: String(localized: "Prefix cache entries"),
            explainer: String(localized: "Hot prefix cache size: how many separate KV snapshots to keep across requests. Lets multi-turn chats skip re-prefilling shared system prompts. 0 disables. Auto-capped on RAM-limited Macs (a 16 GB Mac caps to 1) — on hybrid SSM models each entry pins large KV + state snapshots that can otherwise fill memory."),
            needsRestart: true),
        "prefixCacheMem": .init(
            title: String(localized: "Prefix cache memory cap"),
            explainer: String(localized: "Maximum RAM for the prefix cache. Accepts '2GB', '512MB', '0' (disable byte cap). Default 2GB."),
            needsRestart: true),
        "enablePrefixCacheDisk": .init(
            title: String(localized: "SSD prefix cache"),
            explainer: String(localized: "Persist seen KV prefixes to disk (~/.mlx-serve/kv-cache) so they survive restarts + RAM evictions — turns a cold 30-50s long-context first-token wait into a fast SSD read. OFF by default because it can use many gigabytes of disk."),
            needsRestart: true),
        "prefixCacheDisk": .init(
            title: String(localized: "SSD cache size"),
            explainer: String(localized: "Disk budget for the SSD prefix cache when enabled. Accepts '10GB', '2GB', etc. LRU-evicted to this cap. Only used when 'SSD prefix cache' is on."),
            needsRestart: true),
        "maxResidentMemGB": .init(
            title: String(localized: "Model memory cap"),
            explainer: String(localized: "Total RAM the server will hold in loaded models before it evicts one — or refuses the load when there is nothing to evict. Auto = 80% of what Metal recommends for this Mac. Raise it when a model you know fits is refused with \"not enough memory\" on an idle server; the refusal in the log names the estimate it used. Passes --max-resident-mem."),
            needsRestart: true),
        "skipMemPreflight": .init(
            title: String(localized: "Skip memory pre-flight check"),
            explainer: String(localized: "Bypass the safety check that refuses to load an MLX model when free RAM looks too low for its weights plus warmup headroom. The check is conservative — macOS reclaims file cache as the model loads — so turn this on if a load you know fits is being refused. A genuine over-commit can hard-crash the server. Passes --skip-mem-preflight."),
            needsRestart: true),
        "llamaKvQuant": .init(
            title: String(localized: "KV cache quantization"),
            explainer: String(localized: "llama.cpp's KV-quant scheme (ggml Q8_0 / Q4_0). Same trade as the MLX version: less KV RAM, slower decode — leave off for maximum tokens/sec if you have plenty of RAM. Distinct kernels from the MLX KV-quant; auto-enables flash-attn on the server side when non-default."),
            needsRestart: true),
        "llamaCacheEntries": .init(
            title: String(localized: "Session cache entries"),
            explainer: String(localized: "How many independent llama.cpp KV contexts to keep resident. 1 = legacy single-session (every flip between long prompts evicts the other). >1 keeps the N most-recently-used prompts hot — alternating multi-doc workloads stop cold-prefilling on every flip."),
            needsRestart: true),
        "ssdStreaming": .init(
            title: String(localized: "SSD weight streaming"),
            explainer: String(localized: "Stream DeepSeek-V4-Flash expert weights from SSD instead of holding the whole model in RAM (skips full residency + warmup). Turn on when the model is larger than available memory — it loads instead of OOMing, trading some decode speed for the disk reads. ds4 / DeepSeek-V4-Flash only; ignored by the MLX and llama.cpp engines. Passes --ssd-streaming."),
            needsRestart: true),
        "tokenizeCacheEntries": .init(
            title: String(localized: "Tokenize cache entries"),
            explainer: String(localized: "Per-LoadedModel LRU of chat-template render+tokenize results. Skips re-rendering identical message lists on warm reuse — drops warm tokenize_ms from ~240 ms to ~0 on long-prompt repeats. Applies to all engines. 0 disables."),
            needsRestart: true),
    ]
    ...
    static let requestDefaultFields: [String: ServerOptionField] = [
        "defaultMaxTokens": .init(
            title: String(localized: "Max tokens"),
            explainer: String(localized: "Max tokens to generate per chat turn. \"Auto\" pegs it to the remaining context window — the safe choice on a small-RAM / small-context machine. Per-message overrides win when set."),
            needsRestart: false),
        "defaultTemperature": .init(
            title: String(localized: "Temperature"),
            explainer: String(localized: "0 = deterministic greedy. 0.6–1.0 typical chat. Above 1.0 gets erratic. Applies to the app's chats immediately; also becomes the server default for external clients that omit temperature (Claude Code) after a restart."),
            needsRestart: true),
        "defaultTopP": .init(
            title: String(localized: "Top-p"),
            explainer: String(localized: "Nucleus sampling threshold. 0.95 keeps all but the long tail. 1.0 disables top-p filtering. Also the server default for external clients that omit top_p (restart needed for that part)."),
            needsRestart: true),
        "defaultTopK": .init(
            title: String(localized: "Top-k"),
            explainer: String(localized: "Cap on candidate tokens per step. 0 = follow the model's own recommendation from generation_config.json (Qwen 3.6: 20, Gemma 4: 64); explicit values override it server-wide after a restart."),
            needsRestart: true),
        "defaultRepeatPenalty": .init(
            title: String(localized: "Repetition penalty"),
            explainer: String(localized: "Penalty multiplier for tokens already in the context. 1.0 = none. 1.1 is a typical anti-repeat setting."),
            needsRestart: false),
        "defaultPresencePenalty": .init(
            title: String(localized: "Presence penalty"),
            explainer: String(localized: "Additive penalty per token already present in the context. 0 = none."),
            needsRestart: false),
        "defaultReasoningBudget": .init(
            title: String(localized: "Reasoning budget"),
            explainer: String(localized: "Max thinking tokens per request. -1 = unlimited. Only applies when thinking is enabled."),
            needsRestart: false),
        "defaultEnableThinking": .init(
            title: String(localized: "Enable thinking"),
            explainer: String(localized: "Default the chat client to send `enable_thinking: true`. Only models with reasoning support honor this."),
            needsRestart: false),
        "perRequestEnablePLD": .init(
            title: String(localized: "Per-request PLD"),
            explainer: String(localized: "Auto = follow the server's --pld setting (and the adaptive gate). On/Off forces it."),
            needsRestart: false),
        "perRequestEnableDrafter": .init(
            title: String(localized: "Per-request drafter"),
            explainer: String(localized: "Auto = follow the server. On/Off forces it. Only meaningful when --drafter is loaded."),
            needsRestart: false),
    ]
}
