...
| `server.zig` | HTTP: `/health`, `/v1/models`, `/v1/chat/completions`, `/v1/completions`, `/v1/messages`, `/v1/responses`, `/v1/embeddings` (encoder-only BERT; whole input array batched into padded masked GPU forwards), `/v1/load-model`, `/v1/unload-model`, `/v1/images/generations`, `/v1/audio/speech`, `/v1/video/generations`, WebSocket on `/v1/responses` |
| `gen.zig` | Unified native media generation (image/audio/video) hosted by the main server. Modality-named engine wrappers (`ImageEngine`/`AudioEngine`/`VideoEngine` — swappable internals); `ImageEngine` is a backend union (`FluxImpl` / `krea.Engine`, dispatched on peeked `model_type`). Per-request handler bodies, `detectModality`/`modalityFromType`/`peekModelType`, `normalizeSize`, stub CPU-state builders. See "Unified media generation" below. |
...
## Unified media generation (image / audio / video)

Image (FLUX.2), audio (Qwen3-TTS), and video (LTX-Video) generation are hosted by the **ONE main server** through the multi-model registry — NOT three standalone `mlx-serve --serve` subprocesses (the old `flux_server.zig`/`tts_server.zig`/`ltx_server.zig` are retired). One process, one port, one memory budget; chat + media coexist.

- **Modality slots** (`src/gen.zig` + `model_registry.zig`): `LoadedModel` has `image_engine`/`audio_engine`/`video_engine` (named by MODALITY — the implementations are swappable internals — never by the implementation). `detectModality`/`modalityFromType` route off `config.json`'s `model_type` (`flux2*`/`krea*`/`qwen3_tts`/`AudioVideo`). `model_discovery.isMediaModelType` accepts these (bypassing the LM quant gate) so they're discoverable + loadable by `/v1/load-model`.
...
- **Guard**: `tests/test_unified_gen.sh` (headless → load-by-path → gen → unload + coexistence). The standalone `tests/test_{image,video}_gen.sh` / `test_tts.sh` still boot with `--model <gendir>` (now via the registry path). Bundle download logic: `MediaBundleTests` (allowlist, recursive-subdir, readiness, preset→bundle mappings).
