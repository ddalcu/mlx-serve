...
- **Mamba2 decode is ONE dispatch per layer** (`mamba2_decode.zig`, mlx-lm's `ssm_kernel` with the dt softplus/clamp folded in; the op chain serves prefill and any geometry it declines). Nemotron MoE decode reads its experts in place (`nemotronMoeDecodeExperts`: `gatherQmv` fc1 → ReLU² → `gatherQmv` fc2, f32 weighting after), never the sort machinery. Bar: fused single-token steps == the chain's prefill, host reference at one token.
