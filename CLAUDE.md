...
- **The Nemotron-H MTP head reads the residual BEFORE the final norm and pays per verify ROW on a MoE trunk** (`MtpModel.layout == .nemotron`, `forwardHybridWith` captures pre-norm `h`; `ModelConfig.mtpDepth` caps auto depth at 2): post-norm input lost a quarter of the accepts, and every extra verify row routes to more experts, so depth 6 ran slower than serial. Verify windows up to 8 rows read experts in place (`nemotron_gather_max_rows`). Guard: `mtp: nextMtp on a Nemotron-H trunk emits the serial greedy stream`.
