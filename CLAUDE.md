...
- **SeedVR2 7B is a different configuration the pack STATES** (`transformer_overrides` → `manifest.configFromJson`, absent = 3B): same tensor names, every layer multimodal, biased GELU MLP, `rope_dim` 64, no output ada; temporal rope offset keys on `rope_on_text`.
- **A restore pass scales with PIXEL AREA and fails as a good-looking 200** (white/grey frames at the Metal edge): `restoreMemoryRefusal` 400s by name on both routes; query-tiled mid attention (`attnQueryChunk`) + `evalA` at full-frame boundaries are the residency. Guard: `tests/test_seedvr2_upscale.sh`.
