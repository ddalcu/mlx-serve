...
# system-prompt prefix. With the legacy single-slot cache, alternating between
# conversations evicts the other's KV after every turn → every request pays
# full prefill. With --prefix-cache-entries 2, both conversations should keep
# their prefix hot across the alternation.
#
# Acceptance: alternating-mode 2nd-turn prefill TPS for conversation A is
# ≥ 2× faster with hot cache than without (since with single-slot, the 2nd-turn
# A request pays cold prefill on a long prompt).
#
...
# isolating cap=1 vs cap=2: with cap=1 and an interleaved B1, A2 finds only
# `system` as the shared prefix; with cap=2 and the same interleave, A2's
# entry-A snapshot covers `system+user1+assistant`, so only the new user text
# needs prefill.
...
run_with_capacity() {
    ...
    #   - cap=1: B1's commit overwrote A1 → A2's lookup against B1 shares only `system`
    #     → cold prefill of (user1+assistant+user2)
...
echo "=== capacity=1 (legacy single-slot) ==="
read -r a1_c1 b1_c1 a2_c1 b2_c1 <<< "$(run_with_capacity 1)"
echo "  walls: A1=${a1_c1}ms B1=${b1_c1}ms A2=${a2_c1}ms B2=${b2_c1}ms"
log_c1="$(cat /tmp/test_prefix_cache.log)"
...
#
#  1. cap=1 path must NOT engage the hot cache (no [hot-cache] log lines).
#  2. cap=2 path must engage the hot cache (≥1 [hot-cache] log line).
#  3. cap=2 A2's lookup must find more shared tokens than cap=1's path
#     because A1's snapshot is retained alongside B1's. We can't directly
#     read shared-tokens from cap=1 (it uses the legacy [cache] log). But
#     we CAN assert the cap=2 log mentions reusing tokens from a non-LRU
#     entry on A2 — i.e. there are ≥ 2 entries in play.

# Assertion 1.
if echo "$log_c1" | grep -qE "\[hot-cache\]"; then
    echo "FAIL: cap=1 unexpectedly engaged hot cache" >&2
    exit 1
fi
echo "PASS: cap=1 does not engage hot cache (single-slot legacy path)"

# Assertion 2.
hits_c2=$(echo "$log_c2" | grep -cE "\[hot-cache\] reused")
...
# multiple entries. Format: "reused N/M tokens from entry K (of L)" where
# L >= 2 confirms multi-entry state.
if ! echo "$log_c2" | grep -qE "\(of [2-9]\)"; then
