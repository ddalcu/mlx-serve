...
if boot; then
    ...
    grep -q "Repetition loop guard: short-cycle cutoff=16 repeats" "$LOG"
    check "default loop cutoff remains 16" "$([ $? -eq 0 ] && echo 1 || echo 0)"
else
...
echo "[6/6] repeat-loop CLI values reach the scheduler"
if boot --loop-repetitions 64; then
    grep -q "Repetition loop guard: short-cycle cutoff=64 repeats" "$LOG"
    check "--loop-repetitions 64 reaches the scheduler" "$([ $? -eq 0 ] && echo 1 || echo 0)"
else
    check "boot with --loop-repetitions 64" 0
fi
if boot --loop-repetitions 0; then
    grep -q "Repetition loop guard: disabled (all tiers)" "$LOG"
    check "--loop-repetitions 0 disables the scheduler guard" "$([ $? -eq 0 ] && echo 1 || echo 0)"
else
    check "boot with --loop-repetitions 0" 0
fi
